 ...
 * @return  array   配送方式数组
 */
function available_shipping_list_v2($region_id_list)
{
	$sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        foreach ($goods as $key=>$v)
        {
        	...
        	$temp = explode("*", $retule['goods_volume']);
        	$total['c'] = ($temp[0]);
        	$total['k'] = ($temp[1]);
    ...
    $total['shipping_fee_formated'] = getFreight($order['shipping_id'],$total,$region['country']);
    ...
    $total['shipping_fee_formated']  = price_format($total['shipping_fee_formated'], false);
