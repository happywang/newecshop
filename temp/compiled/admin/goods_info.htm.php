...
<?php echo $this->smarty_insert_scripts(array('files'=>'../js/utils.js,selectzone.js,colorselector.js,../js/jquery.min.js')); ?>
      ...
      <form enctype="multipart/form-data" action="" method="post" name="theForm" >
        ...
        <table width="90%" id="general-table" align="center">
          ...
          <!-- <tr>
            ...
            <span class="notice-span" <?php if ($this->_var['help_open']): ?>style="display:none" <?php else: ?> style="display:none" <?php endif; ?> id="noticeGoodsSN"><?php echo $this->_var['lang']['notice_goods_sn']; ?></span></td>
          </tr> -->
          <tr>
            <td class="label"><?php echo $this->_var['lang']['lab_goods_cat']; ?></td>
            <td><select name="cat_id" id="cat_id" onchange="hideCatDiv()" ><option value="0"><?php echo $this->_var['lang']['select_please']; ?></option><?php echo $this->_var['cat_list']; ?></select>
...
?>
        ...
        <?php echo $this->html_options(array('options'=>$this->_var['suppliers_list_name'],'selected'=>$this->_var['goods']['suppliers_id'])); ?>
         ...
         <?php endif; ?>
          ...
          <tr>
            <td class="label">货源号：</td>
            <td><input type="text" name="goods_source" id="goods_source" value="<?php echo htmlspecialchars($this->_var['goods']['goods_source']); ?>" size="20" onBlur="getGoodsn(this.value);" />
          ...
          <tr>
            <td class="label">
            <!-- <a href="javascript:showNotice('noticeGoodsSN');" title="<?php echo $this->_var['lang']['form_notice']; ?>"><img src="images/notice.gif" width="16" height="16" border="0" alt="<?php echo $this->_var['lang']['form_notice']; ?>"></a> --> <?php echo $this->_var['lang']['lab_goods_sn']; ?> </td>
            <td><input type="text" name="goods_sn" id="goods_sn" value="<?php echo htmlspecialchars($this->_var['goods']['goods_sn']); ?>" size="20" /><span id="goods_sn_notice"></span><br />
            <span class="notice-span" style="display:block">商品货号的组成:(时间+*+供货商号+*+分类号+*+货源号+*+四位随机数)如:20120713112230*23*12*234*1234</span></td>
          </tr>
         ...
         <tr>
            ...
            <td><input type="text" name="source_price" id="source_price" value="<?php echo $this->_var['goods']['source_price']; ?>" size="20" onblur="set_source_price(this.value)"/>
         ...
         <!-- 进价 add by wanghl -->
          <tr>
            ...
            <td><input type="text" name="shop_price" id="shop_price" value="<?php echo $this->_var['goods']['shop_price']; ?>" size="20"/>
            <!-- <input type="button" value="<?php echo $this->_var['lang']['compute_by_mp']; ?>" onclick="marketPriceSetted()" /> -->
            </td>
          </tr>
...
?>
            ...
            <td>
                            注册用户<span id="nrank_1"></span><input type="text" size="8" onkeyup="if(parseInt(this.value)&lt;-1){this.value='-1';};set_price_note(1)" value="" id="reg_price" name="user_price[]" id="rank_1">
              <input type="hidden" value="1" name="user_rank[]">
                            代销用户<span id="nrank_3"></span><input type="text" size="8" onkeyup="if(parseInt(this.value)&lt;-1){this.value='-1';};set_price_note(3)" value="" id="vip_price" name="user_price[]" id="rank_3">
              <input type="hidden" value="3" name="user_rank[]">
                            vip<span id="nrank_2"></span><input type="text" size="8" onkeyup="if(parseInt(this.value)&lt;-1){this.value='-1';};set_price_note(2)" id="agent_price" value="" name="user_price[]" id="rank_2">
...
?>
          ...
          <tr>
            ...
            <td><input type="text" name="market_price" id="market_price" value="<?php echo $this->_var['goods']['market_price']; ?>" size="20" />
              <!-- <input type="button" value="<?php echo $this->_var['lang']['integral_market_price']; ?>" onclick="integral_market_price()" /> -->
            </td>
          </tr>
          
          <!-- --------------------add by wanghl -----------------goods_color ,goods_volume,goods_size------------------------- -->
          <tr>
            <td class="label">体积：</td>
            <td><input type="text" name="goods_volume" id="goods_volume" value="<?php echo $this->_var['goods']['goods_volume']; ?>" size="20" />
              <br /><span class="notice-span" style="display:block">商品的体积属性，填入格式：长*宽*高</span>
            </td>
          </tr>
          
          <tr>
            <td class="label">尺寸(size)：</td>
            <td><input type="text" name="goods_size" id="goods_size" value="<?php echo $this->_var['goods']['goods_size']; ?>" size="20" />
              <br /><span class="notice-span" style="display:block">商品的尺寸属性，默认以cm为单位，但是cm可以省略，只用填数值</span>
            </td>
          </tr>
          
          <tr>
            <td class="label">颜色(color)：</td>
            <td><input type="text" name="goods_color" id="goods_color" value="<?php echo $this->_var['goods']['goods_color']; ?>" size="20" />
              <br /><span class="notice-span" style="display:block">商品的颜色属性，若物品有多个颜色,各颜色用*区分,如:green*red*blue</span>
            </td>
          </tr>
          
          
           <!-- --------------------add by wanghl -----------------goods_color ,goods_volume,goods_size------------------------- end-->
          <tr>
...
function getGoodsn(goods_source){
	var goods_source = goods_source;
	var cat_id = document.getElementById('cat_id').value;
	var suppliers_id = $("#suppliers_id").find("option:selected").val(); 
	if(!cat_id || cat_id == 0){
		alert("请先选择商品分类");
		$("#goods_source").val("");
		return false;
	...
	if(!goods_source){
		...
		//var params = 'goods_source=' + goods_source+'&cat_id=' + cat_id + '&suppliers_id= ' + suppliers_id;
	      //Ajax.call('brand.php?is_ajax=1&act=huoy', params, showGoodsn, 'GET', 'JSON');
		 Ajax.call('goods.php?is_ajax=1&act=huoy', 'goods_source=' + goods_source+'&cat_id=' + cat_id + '&suppliers_id= ' + suppliers_id, showGoodsn, "GET", "JSON");
	}
}
function showGoodsn(result){
	if (result.error == '1' && result.message != '')
    {
        alert(result.message);
        return;
    ...
    document.getElementById('goods_sn').value = response;
    return;
}

function set_source_price(source_price){
	var source_price = source_price;
	var cat_id = document.getElementById('cat_id').value;
	if(!cat_id){
		alert("请先选择商品分类！");
		return false;
	}else{
		 Ajax.call('goods.php?is_ajax=1&act=set_source_price', 'source_price=' + source_price+'&cat_id=' + cat_id, get_source_price, "GET", "JSON");
	}
}
function get_source_price(result){
	if (result.ret < 0)
    {
        alert("设置失败！");
        return;
    }
     document.getElementById('shop_price').value = result.shop_price;
     document.getElementById('market_price').value = result.market_price;
     document.getElementById('reg_price').value = result.reg_price;
     document.getElementById('vip_price').value = result.vip_price;
     document.getElementById('agent_price').value = result.agent_price;
     return;
}
