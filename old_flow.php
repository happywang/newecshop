...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        foreach ($consignee_list as $key=>$v)
        {
        	$consignee_list[$key]['country_en_name'] = get_country_en_name($v['country']);
        }
    ...
    else
    {
        ...
        if ($_SESSION['user_id'] > 0)
        {
            ...
            $consignee['country'] = get_countryid($consignee['country']);
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    $consignee['country_en_name'] = get_country_en_name($consignee['country']);
...
//     $shipping_list     = available_shipping_list($region);
    $shipping_list     = available_shipping_list_v2($region);
...
// 	$return = array();
// 	$return['total_price'] = $cart_goods['total']['goods_price'];
// 	$return['save_rate'] = $cart_goods['total']['save_rate'];
// 	$return['id'] = $_POST['id'];
// 	$return['tmp_num'] = $_POST['tmp_num'];
// 	$return['goods_subtotal'] = '$'.($_POST['tmp_num'] * substr($_POST['goods_price_t'],1));
...
// 	var_dump($cart_goods['total']['goods_price']);
// 	var_dump($cart_goods['total']['save_rate']);exit;
